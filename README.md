...
<!-- 截图位置 1：读取成功后的顶部区域，要同时拍到“读取存档”、因子数量、读取时间和计算设置。建议文件：docs/images/readme/01-inventory-loaded.png -->
...
<!-- 截图位置 2：完整的目标编辑区，左侧显示技能池，右侧显示一个已激活的目标区域、重复目标和灰色不可选技能。建议文件：docs/images/readme/02-target-editor.png -->
...
<!-- 截图位置 3：顶部计算设置，展示计算时间、快速计算内存和“开始分析”按钮。建议文件：docs/images/readme/03-solver-settings.png -->
...
因子列表下方按技能汇总实际等级，例如两条 15 级追击显示为“追击 Lv 30”。页面不再显示所有因子的等级总和。
...
3. 每个选项会显示技能、因子等级和库存位置，选中后整张卡会切换到对应的真实因子。

卡片右上角的铅笔按钮用于更换整枚因子，垃圾桶按钮用于删除。当前方案不足 12 枚因子时，列表末尾会出现“添加因子”。

替换候选会排除当前方案已经使用的其他因子，以及其他角色已确认配装占用的因子。两个词条和等级完全相同的因子仍按库存位置分别处理，不会一次占用同类因子的全部库存。

修改后，目标完成情况、警告和技能等级汇总会立即更新，但原来的前十名排序不会改变。页面会标记“手动调整版”，也可以点击“恢复计算结果”撤销当前方案下的全部手动修改。如果调整后缺少必须满足的技能，或加入了“不能出现”的技能，这套配装不能确认。

<!-- 截图位置 5：建议做一张拼图，左侧展开某个副词条选择框，右侧打开“更换整枚因子”窗口；画面中要能看到库存位置和“手动调整版”提示。建议文件：docs/images/readme/05-manual-adjustment.png -->
...
手动调整过的方案会按调整后的库存位置确认。为其他角色计算时，已占用的因子会自动排除。“已确认配装”页面可以集中查看和取消占用。如果重新读取存档后某枚因子消失，工具会标记缺失，不会自动换成另一枚同词条因子。

<!-- 截图位置 6：“已确认配装”页面，同时展示两套不同角色配装和具体库存位置。建议文件：docs/images/readme/06-confirmed-loadouts.png -->

### 10. 查看持有因子

“持有因子”页面可以搜索主副词条，并按分类、可用状态、游戏内装备状态和等级筛选或排序。这里显示的是最近一次手动读取后的快照，不是游戏内实时库存。

<!-- 截图位置 7：“持有因子”页面，展示搜索、筛选、紧凑因子卡和占用状态。建议文件：docs/images/readme/07-inventory-page.png -->
