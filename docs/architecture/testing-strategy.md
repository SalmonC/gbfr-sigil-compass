...
- `GBFR-RANK-3` 的禁止技能可行性、三类主词条数量、精确覆盖、基础替代顺序、可选目标满足数、尽量避开出现次数、目标位置、槽位、等级和 seed 逐级验证。
...
- 用真实 401 枚库存和 24 个高频不同可选目标复现组合爆炸边界：必须在 8 秒内受控返回 `solver.resource_limit`，RSS 低于 512 MiB，不得写缓存或留下 Worker。另用 24 项全连接合成库存作为不依赖本机存档的发行回归门禁。
