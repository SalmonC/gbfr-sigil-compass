...
# 2026-07-24：聚合库存与 README 截图

- 创建并推送 `stable-v0.2.3` 稳定标签。
- 新增库存分组、计数占用、确定性展开、数量短缺检测和 v3→v4 工作区迁移。
- 结果词条下拉、整枚替换窗口和持有因子页面已合并相同词条与等级，并显示“可用 N”。
- 所有常规界面、搜索项和排序项已移除库存位置。
- README 已加入用户提供的截图 1–3；手动改单词条和更换整枚因子拆成截图 5、6 两段。
- 技能等级汇总已改为分类排序。
- `npm run verify:release` 通过；macOS Electron 用 401 枚真实库存完成聚合页面和实际求解视觉检查。
- 实现后独立对抗式复审完成；三个阻断边界均已修复并加入回归测试。

# 2026-07-24：纯数量库存迁移

- 主进程在存档解析完成后立即把 RawSigil 聚合为库存组；本地快照、Renderer 和工作区不再保存存档位置。
- 求解器和手动调整改用逻辑因子，确认配装继续按 `groupKey + count` 扣减。
- 工作区升级到 v5；加入 v4 迁移回归，逐字段确认命名方案与目标设置不变，旧计算缓存被清理。
- 库存快照升级到 schema 2，并加入旧 schema 1 聚合迁移和“磁盘文件不含 inventorySlotId”断言。
- `npm run verify:release` 通过；真实存档性能回归通过。
